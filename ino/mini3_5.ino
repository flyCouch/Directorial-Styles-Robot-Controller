...
const int dirPin2 = 5;
const int stepPin3 = 7; //diff for mini3
const int dirPin3 = 8;
...
const float WHEEL_RADIUS = 0.024;  // in meters
const float ROBOT_RADIUS = 0.115;  // in meters
const float STEPS_PER_REV = 800;  // steps per revolution
...
const unsigned long MIN_STEP_DELAY = 100; // e.g., 500 us for 2000 SPS
...
void loop() {
  if (radio.available()) {
    ...
    float scaled_x = receivedData.x * SCALE_FACTOR;
    float scaled_y = receivedData.y * SCALE_FACTOR;
    float scaled_r = receivedData.r * SCALE_FACTOR; // If r is already rad/s, no need to multiply by 5

    scaled_r = scaled_r * 2 * 3.1459;
    ...
    const unsigned long MIN_ACCEPTABLE_DELAY = 200; // Smallest delay (fastest speed, e.g., 2000 SPS)
