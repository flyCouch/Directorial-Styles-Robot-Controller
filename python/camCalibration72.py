...
class MJPEGFrameReader:
    def __init__(self, url):
        ...
        self.stop_flag = False  # NEW: allow cooperative stop
    ...
    def open(self):
        ...
        try:
            ...
            # Try to set a low-level socket timeout (if available)
            try:
                self.stream.fp.raw._sock.settimeout(3)
            except Exception:
                pass

            # Try to read the first boundary, but bail out if it takes too long
            start_time = time.time()
            header = b''
            while True:
                if time.time() - start_time > 3:
                    raise TimeoutError("Stream boundary not found (timeout)")
    ...
    def read(self):
        ...
        if not self.is_open or self.stop_flag:
            return False, None
        ...
        try:
            ...
            while True:
                ...
                if not line:
                    break  # Empty line separates headers from image data
            ...
            # suppress repeated error prints – treat as closed
            self.is_open = False
            return False, None

    def release(self):
        """Closes the underlying stream connection."""
        # Signal read() to stop as soon as it checks stop_flag
        self.stop_flag = True
        try:
            if self.stream:
                self.stream.close()
...
class CameraCalibrationApp:
    def __init__(self, root):
        ...
        self.root.after(100, self.load_stream) 
    ...
    def stream_update_loop(self):
        ...
        while self.is_running:
            cap = None
            # take a quick thread-safe snapshot of cap
            with self.stream_lock:
                cap = self.cap

            if cap and getattr(cap, 'is_open', False) and not getattr(cap, 'stop_flag', False):
                ret, frame = cap.read()
                if ret:
                    self.last_frame = frame
                    self.root.after(0, self.update_display)
                else:
                    # If stream fails mid-read, try to release it and schedule reconnect
                    try:
                        self.release_stream()
                    except Exception:
                        pass
                    # schedule reconnect on main thread, but only if not already connecting
                    if not self.connecting:
                        self.root.after(0, self.load_stream)
                    # small backoff
                    time.sleep(0.15)
            else:
                # no cap available — attempt to start connection on main thread
                if not self.connecting:
                    # schedule connecting on the main thread (idempotent thanks to self.connecting)
                    self.root.after(0, self.load_stream)
                time.sleep(0.1)
    ...
    def load_stream(self):
        """Attempts to connect to the current camera's stream (non-blocking open)."""
        ...
        try:
            # TEMP debug: shows in terminal where we are
            print(">>> load_stream starting")

            # Ensure previous stream is fully released before starting a new one
            self.release_stream()
            time.sleep(0.15)

            full_ip = f"{IP_BASE}.{self.current_ip_end}"
            # Update UI from main thread — load_stream is always scheduled on main thread
            try:
            ...
            for endpoint in STREAM_ENDPOINTS:
                ...
                # Run open() in a short-lived thread and wait with timeout
                open_thread = threading.Thread(target=reader.open, daemon=True)
                open_thread.start()
                open_thread.join(3.0)  # wait up to 3 seconds for open() to complete

                # If thread still alive, try to aggressively close socket so the thread unblocks
                if open_thread.is_alive():
                    # Attempt to close the low-level stream/socket immediately
                    try:
                        # reader.release() sets stop_flag and closes stream if present
                        reader.release()
                    except Exception:
                        pass

                    # Give the thread a short moment to exit after stream close
                    open_thread.join(0.5)

                    # If it's STILL alive, print a warning and continue — we won't hang forever here
                    if open_thread.is_alive():
                        print(f"Warning: open() thread still alive for {url} after forced release. Continuing.")

                # Check if the reader succeeded
                if getattr(reader, "is_open", False):
                    stream_reader = reader
                    found_endpoint = True
                    self.display_message(f"SUCCESS: Stream opened on {endpoint}.")
                    break
                else:
                    # Ensure reader is closed if partially opened or timed out
                    try:
                        reader.release()
                    except Exception:
                        pass
    ...
    def release_stream(self):
        """Closes the current stream connection cleanly."""
        # Ensure only one thread tries to release at a time
        with self.stream_lock:
            if self.cap:
                try:
                    # Ask the reader to stop any blocking read asap
                    self.cap.release()
                except Exception:
                    pass
                # drop reference so update loop doesn't try to read it again
                self.cap = None
    ...
    def reset_for_new_cam(self):
        ...
        self.release_stream()
        # give the OS/sockets a small moment to close
        time.sleep(0.15)
