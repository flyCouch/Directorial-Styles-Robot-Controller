...
class JoystickConfiguratorApp:
    ...
    def _populate_mapping_rows(self):
        ...
        for i, action_name in enumerate(ACTIONS):
            ...
            # REMOVED: These grid_columnconfigure calls are now handled once in _setup_ui
            # self.action_rows_frame.grid_columnconfigure(0, weight=1)
    ...
    def _handle_joystick_events(self):
        ...
        if self.joystick_manager.active_joystick and not self.assignment_mode: # Only send data if not in assignment mode
            ...
            # Condition 2: Any analog axis is active (non-zero beyond threshold), force continuous send
            if abs(command_payload["x"]) > ANALOG_AXIS_THRESHOLD or \
               abs(command_payload["y"]) > ANALOG_AXIS_THRESHOLD or \
               abs(command_payload["r"]) > ANALOG_AXIS_THRESHOLD or \
               abs(command_payload["e"]) > ANALOG_AXIS_THRESHOLD:
                should_send = True
