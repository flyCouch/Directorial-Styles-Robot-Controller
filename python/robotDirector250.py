...
class robotDirector:
    ...
    def __init__(self, master): 
        ...
        # ...
        self.ROBOT_MAX_LINEAR_VELOCITY_MM_PER_SEC = 1.0 # Adjust this to your robot's actual max linear speed in mm/sec
    ...
    def _update_gcode_feed_rate_from_slider(self, value):
        ...
        self.gcode_current_feed_rate = new_speed_multiplier * self.ROBOT_MAX_LINEAR_VELOCITY_MM_PER_SEC * 60
    ...
    def send_control_command(self):
        ...
        try:
            ...
            command_string = (
                ...
                f"MY:{motion_y:.2f},"
                f"R:{rotation:.2f},"
                f"L:{laser},"        # Use the 'laser' variable we just got from motion_command
                f"P:{laser_power},"  # Use the 'laser_power' variable we just got from motion_command
    ...
    def load_gcode_file(self, filepath):
        ...
        try:
            ...
            if parsed_lines:
                ...
                self.gcode_current_feed_rate = self.ROBOT_MAX_LINEAR_VELOCITY_MM_PER_SEC * 60 
    ...
    def _process_next_gcode_command(self):
        ...
        # Regex for G1/G0 (Movement) commands, including optional X, Y, Z, F, S
        # Note: Added Z for completeness, even if not used by your robot.
        match_g1_g0 = re.match(r'^(G[01])(?: X([\d.-]+))?(?: Y([\d.-]+))?(?: Z([\d.-]+))?(?: F([\d.]+))?(?: S(\d+))?', gcode_line)
        # Regex for M3/M5 (Laser ON/OFF) commands, including optional S
        match_m3_m5 = re.match(r'^(M[35])(?: S(\d+))?', gcode_line)
        # Regex for standalone S (Laser Power) commands
        s_only_match = re.match(r'^S(\d+)$', gcode_line)
        # Regex for F (Feed Rate) commands
        f_match = re.match(r'^F([\d.]+)$', gcode_line)
        # Regex for G90/G91 (Absolute/Relative Positioning)
        g9x_match = re.match(r'^(G90|G91)$', gcode_line)

        # --- G-code Command Handling Logic ---

        # 1. Handle M3/M5 (Laser ON/OFF) Commands
        if match_m3_m5:
            ...
            if gcode_command == 'M3':
                ...
                self.gcode_current_laser_power = int(s_value_str) if s_value_str else 255 # Default to max if S not specified
            ...
            # Update Tkinter variables for GUI display
            self.laser_on.set(self.gcode_current_laser_on)
            self.current_laser_power.set(self.gcode_current_laser_power)
            
            # Update motion_command for the laser state and send
            self.motion_command["x"] = 0.0 # M3/M5 don't command movement themselves
            self.motion_command["y"] = 0.0
            self.motion_command["rotation"] = 0.0
        ...
        # 2. Handle G1/G0 Commands (Movement, possibly with S for laser)
        elif match_g1_g0:
            ...
            if feed_rate_str:
                self.gcode_current_feed_rate = float(feed_rate_str) # Use gcode_current_feed_rate
                print(f"[DEBUG GCODE] G1/G0 with F value. Current Feed Rate: {self.gcode_current_feed_rate}")
            ...
            if target_x_str is not None and target_y_str is not None: # Check if X and Y are present for movement
                ...
                # --- START OF INTEGRATION FROM robotDirector241.py (Motor Fix) ---
                # G-code feed rates are typically in mm/minute. Convert to m/s.
                base_linear_velocity_m_per_sec = self.gcode_current_feed_rate / 60000.0

                # **CRITICAL FIX:** Scale this velocity up to make motors respond
                # This factor was identified as making motors move reliably in 241.
                gcode_velocity_scale_factor = 50.0 # Adjust this value if robot moves too fast/slow
                linear_velocity_m_per_sec = base_linear_velocity_m_per_sec * gcode_velocity_scale_factor
                
                # Ensure a minimum velocity to prevent motors from stalling at very low speeds
                if linear_velocity_m_per_sec < 0.005: # Set a practical minimum speed, e.g., 0.005 m/s
                    linear_velocity_m_per_sec = 0.005
                print(f"[DEBUG GCODE] Final linear_velocity_m_per_sec: {linear_velocity_m_per_sec:.4f} m/s")
                # --- END OF INTEGRATION ---

                if distance_mm > 0.01: # Only move if there's significant distance
                    # Set motion_command to the desired velocity components in meters/second
                    # This uses the scaled linear_velocity_m_per_sec
                    self.motion_command["x"] = (dx / distance_mm) * linear_velocity_m_per_sec
                    self.motion_command["y"] = (dy / distance_mm) * linear_velocity_m_per_sec
                    self.motion_command["rotation"] = 0.0 # Assuming G1/G0 directly commands linear movement

                    # Calculate duration of movement for scheduling next command
                    move_duration_sec = distance_mm / (linear_velocity_m_per_sec * 1000) if linear_velocity_m_per_sec > 0 else 0 # Convert m/s back to mm/s for duration calc
                    delay_ms = int(move_duration_sec * 1000)
                    delay_ms = max(50, delay_ms) # Minimum 50ms to ensure a command is sent/processed

                    print(f"[DEBUG GCODE] Scheduled move duration: {delay_ms}ms.")
                    self.master.after(delay_ms, self._stop_robot_and_continue_gcode)
                    
                else: # No significant movement
                    print("[DEBUG GCODE] G0/G1: No significant movement (distance < 0.01mm). Skipping actual move.")
                    # If no move, continue processing next command immediately
                    if self.gcode_processing_active:
                        self.master.after(10, self._process_next_gcode_command)
                    
                # Update current position after calculating dx, dy
                self.gcode_current_x = target_x
                self.gcode_current_y = target_y

            else: # No X, Y in G0/G1 command, or only one axis specified
                # If only F or S, or no movement, ensure motion commands are zero
                self.motion_command["x"] = 0.0
                self.motion_command["y"] = 0.0
                self.motion_command["rotation"] = 0.0
                # If no movement, continue processing next command immediately
                if self.gcode_processing_active:
                    self.master.after(10, self._process_next_gcode_command)
            ...
            self.send_control_command() # Send command to Arduino (even if just laser state updated)
            return # Exit this call, wait for the scheduled continuation or next command
        ...
        # 3. Handle Standalone S Commands (Laser Power)
            ...
            if s_value == 0:
                self.gcode_current_laser_on = False # S0 means laser off
            else:
                self.gcode_current_laser_on = True # S > 0 means laser on
            ...
            # Update motion_command for laser state (no movement for this specific G-code line)
            self.motion_command["x"] = 0.0
            self.motion_command["y"] = 0.0
            self.motion_command["rotation"] = 0.0
            self.motion_command["laser_on"] = self.gcode_current_laser_on
            ...
            # Schedule next command (no _stop_robot_and_continue_gcode needed as no movement)
            if self.gcode_processing_active:
                self.master.after(10, self._process_next_gcode_command)
            return
        ...
        # 4. Handle F Commands (Feed Rate)
            ...
            self.gcode_current_feed_rate = feed_rate # Update current feed rate
            print(f"[DEBUG GCODE] Matched F command. Feed Rate set to: {self.gcode_current_feed_rate}")
            self.update_gcode_status(f"Feed Rate: {feed_rate}")
            # F commands don't trigger immediate movement or laser send; just update internal state
            if self.gcode_processing_active:
                self.master.after(10, self._process_next_gcode_command)
            return

        # 5. Handle G90/G91 (Absolute/Relative Positioning)
        elif g9x_match:
            g_command = g9x_match.group(1)
            if g_command == 'G90':
                self.gcode_absolute_mode = True
            elif g_command == 'G91':
                self.gcode_absolute_mode = False
            print(f"[DEBUG GCODE] Matched {g_command} command. Positioning Mode: {'Absolute' if self.gcode_absolute_mode else 'Relative'}.")
            self.update_gcode_status(f"Positioning Mode: {g_command}")
            if self.gcode_processing_active:
                self.master.after(10, self._process_next_gcode_command)
            return

        # 6. Catch-all for Unrecognized G-code
        else: # This block will now only hit truly unrecognized commands
    ...
    def _stop_robot_and_continue_gcode(self):
        # ... (This method remains largely the same, ensure it calls send_control_command) ...
