...
class robotDirector:
    ...
    def _process_tag_data(self, data):
        ...
        try:
            ...
            # --- V12 DEBUG: Add print statement for uncalibrated state ---
                ...
                print("DEBUG: Vision data received but CNC position not updated (Not Calibrated).") 
            # -------------------------------------------------------------
    ...
    def update_cnc_position(self, T_x, T_y, T_r):
        ...
        CNC Pos = Raw Pos + Offset
        """
        if not self.is_vision_calibrated.get():
            return
            
        try:
            O_x = self.vision_offset_x.get()
            O_y = self.vision_offset_y.get()
            O_r = self.vision_offset_r.get()
            ...
            # --- DEBUG V12: Confirm update is happening ---
            print(f"DEBUG V12: CNC Position Update. Raw: ({T_x:.2f}, {T_y:.2f}). Offset: ({O_x:.2f}, {O_y:.2f}). CNC: ({C_x:.2f}, {C_y:.2f})")
            # ----------------------------------------------
    ...
    def calibrate_vision_origin(self):
        ...
        C_x = self.work_area_width_mm.get() / 2.0
        C_y = self.work_area_height_mm.get() / 2.0
        ...
        self.vision_offset_x.set(f"{offset_x:.2f}")
        self.vision_offset_y.set(f"{offset_y:.2f}")
        self.vision_offset_r.set(f"{offset_r:.2f}")
                    ...
                    if m:
                        ...
                        if g_code in [0, 1]:
                            ...
                            if self.is_vision_calibrated.get() and self.tag_thread_running:
                                ...
                                raw_target_x = target_x - self.vision_offset_x.get()
                                raw_target_y = target_y - self.vision_offset_y.get()
